...
return(
  ...
  <tr><td>New workers</td> <td>{nworkers}</td></tr>
    ...
    const rebateRecords = ()=>{
      ...
      for (let x=0;x<records.length;x++){
        let section= records[x].data.section
        let type = records[x].data.type
        let amount= records[x].data.amount
        let desc= records[x].data.desc
...
<p className="introtext">Rebate account balance: {rebateIntro()} <br/> Current month remittance: {introRemitCalc()}</p>

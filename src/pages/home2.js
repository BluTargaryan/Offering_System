...
const Home = ()=>{
 ...
 const [recordsdu, setRecordsDu] = useState([])
   ...
   const [period, setPeriod] = useState(0)
    ...
    const inViewport = useIntersection(introRef2, '-200px');
...
const generatePeriod = async()=>{
 ...
 useEffect(() => {
  ...
  useEffect(()=>{
    ...
    const recordsRef = collection(db, 'recordsdu');
...
try{
  ...
  document.getElementById('general').style.display="flex"
    ...
    const toDelete = async() =>{
        const recordsDocRef = doc(db, 'recordsdu', denoteId)
        try{
...
return(
  ...
  <tr><td>New workers</td> <td>{nworkers}</td></tr>
    ...
    const rebateRecords = ()=>{
      ...
      for (let x=0;x<recordsdu.length;x++){
        let section= recordsdu[x].data.section
...
<Main id="main">
   <Up id="arrow">
  <HashLink smooth to='/home2/#main' ><img src={up} alt="up arrow" /></HashLink>
  </Up>
<div className="intro"  id="buttons" ref={introRef2}>
